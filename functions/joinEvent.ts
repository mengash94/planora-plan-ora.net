...
import { createNotification } from '../components/instabackService.js';
...
Deno.serve(async (req) => {
  try {
    ...
    // 5) Ensure membership (service role)
    ...
    const result = { success: true, eventData: event, user: user };

    if (result.success) {
      try {
        const eventDetails = result.eventData || {};
        
        await createNotification({
          userId: result.user?.id,
          type: 'event_invitation',
          title: 'הצטרפת לאירוע',
          message: `ברוך הבא לאירוע "${eventDetails.title || eventDetails.name || 'אירוע'}"`,
          eventId: eventId,
          actionUrl: `/EventDetail?id=${eventId}`,
          priority: 'high'
        });
      } catch (notificationError) {
        console.warn('Failed to create join event notification:', notificationError);
      }
    }

    // Return the full result object as a JSON response
